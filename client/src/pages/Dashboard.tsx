            ...
            <form onSubmit={handleConnect} className="space-y-2">
              ...
              {oauth && extraFields ? (
                <>
                  ...
                  {(() => {
                    const isGoogle = /^google|^gmail$/.test(serviceKey);
                    return (
                      <button
                        type="button"
                        onClick={handleOAuthStart}
                        disabled={busy}
                        className={`w-full ${isGoogle ? "bg-blue-600 hover:bg-blue-700" : "bg-accent-600 hover:bg-accent-700"} disabled:opacity-50 text-white text-sm font-medium py-2.5 px-4 rounded-lg transition flex items-center justify-center gap-2`}
                      >
                        {isGoogle && (
                  ...
                  <div className="pt-1">
                    <button
                      ...
                      className="text-xs text-theme-secondary hover:text-theme-primary transition"
                    >
                      {showManualToken ? "Hide manual entry" : `Or paste a${tokenLabel?.toLowerCase().includes("refresh") ? " refresh" : "n access"} token manually`}
                    </button>
                  </div>
                  {showManualToken && (
                    <>
