...
| Google Ads | OAuth + dev token (auto-refresh) | Google OAuth + developer token | `googleads.googleapis.com/v20/customers:listAccessibleCustomers` |
...
| Gmail | Bearer (OAuth, auto-refresh) | Google OAuth + client credentials | `gmail.googleapis.com/gmail/v1/users/me/profile` |
...
- Google Ads / Gmail: OAuth with auto-refresh — dashboard stores refresh token + client credentials; `preConnect` hook exchanges for fresh access token on each API call. No manual re-auth needed.
- QuickBooks: OAuth tokens expire after ~1 hour — `authNote` warning shown in UI
...
For Azure AD services, add a `preConnect` hook that exchanges client credentials for a bearer token. For OAuth services with auto-refresh (like Gmail, Google Ads), add an `oauth` config with scopes/redirect + a `preConnect` hook that uses the stored refresh token.

## API Endpoints

| Method | Endpoint | Purpose |
|--------|----------|---------|
| GET | `/api/services/status` | All services connection state + extras |
...
| POST | `/api/services/:id/oauth/start` | Start OAuth flow (Gmail, Google Ads) |
| GET | `/api/services/:id/oauth/callback` | OAuth redirect handler |
| POST | `/api/services/:id/oauth/reauth` | Re-authorize with stored credentials |
...
| File | Purpose |
|------|---------|
| `server/src/services.ts` | Service configs, auth headers, preConnect hooks |
| `server/src/routes/services.ts` | Service connection/validation/storage/OAuth flows |
